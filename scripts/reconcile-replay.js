...
exports.HorizonFixtureClient = void 0;
exports.runReconcileReplayCli = main;
exports.fetchHorizonFixture = fetchHorizonFixture;
exports.signReport = signReport;
require("dotenv/config");
const node_crypto_1 = require("node:crypto");
const pg_1 = require("pg");
const revenueReconciliationService_1 = require("../src/services/revenueReconciliationService");
const metrics_1 = require("../src/lib/metrics");
// ---------------------------------------------------------------------------
 ...
 */
class HorizonFixtureClient {
    fixture;
    constructor(fixture) {
        this.fixture = fixture;
    }
    async getRevenueState(_contractAddress) {
        return {
            totalDistributed: this.fixture.totalDistributed ?? '0.00',
        };
    }
}
exports.HorizonFixtureClient = HorizonFixtureClient;
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
/**
 ...
 */
async function fetchHorizonFixture(url) {
    let response;
    try {
        response = await fetch(url);
    }
    catch (err) {
        throw new Error(`Failed to connect to fixture URL "${url}": ${err instanceof Error ? err.message : String(err)}. ` +
            'Verify the URL is reachable and points to a valid Horizon snapshot.');
    }
    if (!response.ok) {
        throw new Error(`Fixture URL returned HTTP ${response.status}${response.statusText ? ` ${response.statusText}` : ''}. ` +
            'The archived Horizon snapshot may be missing or inaccessible. ' +
            `Check that the fixture exists at "${url}".`);
    }
    const body = await response.text();
    if (!body || body.trim().length === 0) {
        throw new Error(`Fixture URL "${url}" returned an empty response body. ` +
    ...
    try {
        ...
        throw new Error(`Fixture URL "${url}" returned non-JSON content. ` +
...
async function main() {
    ...
    try {
        ...
        metrics_1.globalMetrics.incrementCounter('reconciliation_replay_errors_total', {
    ...
    if (!signingSecret) {
        console.error('Error: REPLAY_SIGNING_SECRET is required to sign the replay report. ' +
    ...
    try {
        ...
        const service = new revenueReconciliationService_1.RevenueReconciliationService(pool, fixtureClient);
        console.error(`Running reconciliation for offering ${offeringId} ` +
            `from ${formatDate(periodStart)} to ${formatDate(periodEnd)}...`);
        const reconciliation = await service.reconcile(offeringId, periodStart, periodEnd, { checkInvestorAllocations: false, checkRoundingAdjustments: false });
        ...
        // Record metrics
        metrics_1.globalMetrics.incrementCounter('reconciliation_replay_completed_total', {
            offering_id: offeringId,
            is_balanced: String(reconciliation.isBalanced),
        });
        metrics_1.globalMetrics.setGauge('reconciliation_replay_discrepancies', reconciliation.discrepancies.length, { offering_id: offeringId });
        const hasCriticalErrors = reconciliation.discrepancies.some((d) => d.severity === 'critical');
        const hasErrors = reconciliation.discrepancies.some((d) => d.severity === 'error');
        console.error('');
        ...
        console.error(`Error: Reconciliation replay failed: ${err instanceof Error ? err.message : String(err)}`);
        metrics_1.globalMetrics.incrementCounter('reconciliation_replay_errors_total', {
