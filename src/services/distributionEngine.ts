...
function calculateDistributionPayouts(
  ...
  const totalBalanceDecimal = balanceDecimals.reduce(
    (sum, bd) => sum.add(bd),
    new Decimal("0"),
  );
  ...
  const rawShares: RawShare[] = balances.map((b, index) => {
    ...
    const share = balanceDecimal
      .divide(totalBalanceDecimal)
      .multiply(revenueDecimal);
...
class DistributionEngine {
  ...
  constructor(
    ...
    private balanceProvider?: {
      getBalances: (offeringId: string, period: any) => Promise<BalanceRow[]>;
    },
  ...
  /**
   * Public entry point.  Acquires a transaction-scoped Postgres advisory lock
   * keyed on (offeringId, period.id) before invoking the inner batch so that
   * concurrent callers — two scheduler instances or a manual POST colliding
   * with the automated scheduler — cannot both run the same distribution and
   * double-pay investors.
   *
   * Lock acquisition is *non-blocking*.  If another process already holds the
   * lock, this method throws `Errors.conflict` (HTTP 409) via the structured
   * `AdvisoryLockNotAvailableError` so the caller can decide whether to skip
   * (scheduler) or report contention to the client (HTTP handler).
   *
   * When no `pool` is configured the lock step is skipped entirely — this is
   * the test / legacy path and must never be used in production.
   *
  ...
  > {
    if (!period || !period.id) {
      throw Errors.badRequest("Valid distribution period with ID is required");
    }
    ...
    if (!this.pool) {
      ...
      batchResult = await this.distributeWithBatch(
        offeringId,
        period,
        revenueAmount,
      );
      ...
      try {
        batchResult = await withAdvisoryLock<DistributionBatchResult>(
          this.pool,
          offeringId,
          period.id,
          async () => {
            // Advisory lock is held for the entire batch duration.
            // NOTE: distributeWithBatch will open its own inner per-batch
            // transactions for payout persistence; the outer transaction we
            // run inside here exists purely to scope the advisory lock and
            // guarantee exclusive ownership for the full duration.
            return await this.distributeWithBatch(
              offeringId,
              period,
              revenueAmount,
            );
        ...
        if (err instanceof TransactionError) {
          if (err.cause instanceof AdvisoryLockNotAvailableError) {
        ...
        if (err instanceof AdvisoryLockNotAvailableError) {
          throw Errors.conflict(
            `Distribution for offering ${offeringId} / period ${period.id} is already in progress`,
          );
        }
    ...
    if (batchResult.failedPayouts.length > 0) {
      ...
      throw new Error(
        `Distribution failed: ${firstFailure.errorClass ?? "UNKNOWN"}`,
      );
    }
  ...
  async distributeWithBatch(
    ...
    if (revenueAmount <= 0)
      throw Errors.badRequest("revenueAmount must be > 0");
    if (!period || !period.id || !period.end)
      throw Errors.badRequest("Valid distribution period with ID is required");

    const amtStr = revenueAmount.toFixed(2);

    // 2. Idempotency Check: Look for an existing run
    let run = await this.distributionRepo.findRunByParams(
      offeringId,
      period.id,
      amtStr,
    );
    ...
    if (run) {
      if (run.status === "completed") {
        this.logger.info(
          "Distribution already completed, returning cached results",
          {
            offeringId,
            periodId: period.id,
            runId: run.id,
        ...
        const existingPayouts = await this.distributionRepo.getPayoutsForRun(
          run.id,
        );
        return {
          distributionRun: run,
          successfulPayouts: existingPayouts.map((p: any) => ({
    ...
    try {
      balances = await this.withRetry(() =>
        this.fetchBalances(offeringId, period),
      );
    } catch (err) {
      ...
      throw Errors.serviceUnavailable(
        `Failed to acquire balances: ${failure.class}`,
      );
    }
    ...
    let frozenFxRateId: string | undefined =
      run?.frozen_fx_rate_id || undefined;
    if (this.fxConversionEngine) {
      try {
        ...
        if (
          this.auditLogRepo &&
          typeof this.auditLogRepo.createAuditLog === "function"
        ) {
        ...
        this.logger.warn(
          "Failed to freeze FX rate, continuing without freeze",
          {
            offeringId,
            periodId: period.id,
    ...
    if (!run) {
      try {
        ...
        throw Errors.internal(
          `Failed to initialize distribution run: ${failure.class}`,
        );
      }
      ...
      try {
        ...
        throw Errors.internal(
          `Failed to update distribution status: ${failure.class}`,
        );
      }
    }
    ...
    const existingInvestorIds = new Set(
      existingPayouts.map((p: any) => p.investor_id),
    );

    const successfulPayouts: Array<{ investor_id: string; amount: string }> =
      existingPayouts.map((p: any) => ({
        investor_id: p.investor_id,
        amount: p.amount,
      }));
    const failedPayouts: Array<{
      investor_id: string;
      amount: string;
      error: string;
      errorClass?: string;
      ...
      try {
        ...
        if (!this.pool) {
          for (const r of batch) {
            ...
            if (
              !existingInvestorIds.has(r.investor_id) &&
              !successfulPayouts.some((p) => p.investor_id === r.investor_id)
            ) {
              const amtStr = r.amount.toString();
              failedPayouts.push({
                investor_id: r.investor_id,
    ...
    const finalStatus =
      failedPayouts.length === 0 && !hasBatchFailure ? "completed" : "failed";
  ...
  private async fetchBalances(
    ...
    if (
      ...
      typeof this.balanceProvider.getBalances === "function"
    ) {
      return await this.balanceProvider.getBalances(offeringId, period.id);
    } else if (
      this.offeringRepo &&
      typeof this.offeringRepo.getInvestors === "function"
    ) {
      return await this.offeringRepo.getInvestors(offeringId, period);
    } else if (
      this.offeringRepo &&
      typeof this.offeringRepo.listInvestors === "function"
    ) {
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        if (attempt < this.maxRetries) {
          const delay =
            this.initialDelayMs * Math.pow(this.backoffFactor, attempt - 1);
          if (this.logRetries) {
            this.logger.warn(
              `[DistributionEngine] Retry attempt ${attempt} failed, retrying in ${delay}ms...`,
            );
          }
  ...
  private async fanOutNotifications(
    ...
    if (
      ...
      !this.notificationPreferencesRepo ||
      !this.pool
    ) {
      this.logger.debug(
        "Skipping notification fan-out due to missing dependencies",
      );
      return;
    }
    ...
    const processNotification = async (
      ...
      const prefs =
        await this.notificationPreferencesRepo.getByUserId(investorId);
      if (
        prefs &&
