...
describe("RuleEvaluator", () => {
  ...
  describe("Structuring Rule Evaluation", () => {
    ...
    it("should detect deposits clustered just under reporting threshold ($9,900 smurfing pattern)", async () => {
      const rule: AMLRule = {
        id: "struct_rule_1",
        name: "Structuring Amount Clustering",
        description: "Detects smurfing deposits clustered under CTR threshold",
        type: "structuring",
        version: { major: 1, minor: 0, patch: 0 },
        severity: "critical",
        enabled: true,
        config: {
          ...
          cluster_lower_ratio: 0.8,
          cluster_upper_ratio: 0.999,
          min_cluster_count: 2,
          score_threshold: 50,
          window_days: 30,
        },
      ...
      const now = new Date();
      const context: TransactionContext = {
        investment_id: "inv_smurf_1",
        investor_id: "investor_smurf_1",
      ...
      expect(res.details.clustered_count).toBe(3);
      expect(res.details.clustered_total_amount).toBe(29200);
      expect(typeof res.details.cluster_score).toBe("number");
      expect(res.details.cluster_score as number).toBeGreaterThanOrEqual(50);
      expect(res.details.linked_investment_ids).toEqual(
        expect.arrayContaining(["inv_smurf_2", "inv_smurf_3", "inv_smurf_1"]),
      );
    });

    it("refund protection: refunded or failed transactions do not distort cluster score", async () => {
      const rule: AMLRule = {
        id: "struct_rule_refund",
        name: "Structuring Refund Protection",
        description: "Verifies refunds are ignored in cluster score",
        type: "structuring",
        version: { major: 1, minor: 0, patch: 0 },
        severity: "high",
        enabled: true,
      ...
      const now = new Date();
      const context: TransactionContext = {
        investment_id: "inv_clean_1",
        investor_id: "investor_refund_test",
        offering_id: "offering_1",
        amount: "1000",
        asset: "USD",
        timestamp: now,
      ...
      expect(res.details.clustered_count).toBe(0);
      expect(res.details.cluster_score).toBe(0);
    });

    it("jurisdiction awareness: applies jurisdiction-specific reporting threshold (e.g. JP ¥1,000,000)", async () => {
      const rule: AMLRule = {
        id: "struct_rule_jp",
        name: "Structuring JP Jurisdiction",
        description: "Applies JP Yen reporting threshold",
        type: "structuring",
        version: { major: 1, minor: 0, patch: 0 },
        severity: "high",
        enabled: true,
      ...
      const now = new Date();
      const context: TransactionContext = {
        investment_id: "inv_jp_1",
        investor_id: "investor_jp",
        offering_id: "offering_jp",
        amount: "980000",
        asset: "JPY",
        investor_country: "JP",
        timestamp: now,
      ...
      const res = results[0];
      expect(res.triggered).toBe(true);
      expect(res.details.reporting_threshold).toBe(1000000);
      expect(res.details.jurisdiction).toBe("JP");
      expect(res.details.clustered_count).toBe(2);
    });

    it("emits aml.structuring.score gauge metric via MetricsCollector", async () => {
      const metrics = new MetricsCollector({ enabled: true });
      const evaluatorWithMetrics = new RuleEvaluator(
        mockRepo as unknown as InvestmentRepository,
        { metrics },
      );

      const rule: AMLRule = {
        id: "struct_metric_rule",
        name: "Structuring Metric Test",
        description: "Verifies gauge metric emission",
        type: "structuring",
        version: { major: 1, minor: 0, patch: 0 },
        severity: "high",
        enabled: true,
      ...
      const context: TransactionContext = {
        investment_id: "inv_m_1",
        investor_id: "investor_metric_user",
        offering_id: "off1",
        amount: "9500",
        asset: "USD",
        timestamp: new Date(),
      ...
      await evaluatorWithMetrics.evaluate(context, [rule]);
      const snapshot = await metrics.getSnapshot(
        null as unknown as Parameters<typeof metrics.getSnapshot>[0],
      );
      const gauge = snapshot.custom.find(
        (p) => p.name === "aml_structuring_score",
      );
      expect(gauge).toBeDefined();
      expect(gauge?.value).toBeGreaterThanOrEqual(0);
    });

    it("fast-check property-based test: cluster score is always bounded [0, 100]", async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.array(
            fc.record({
          ...
          async (sampleTxs) => {
            const rule: AMLRule = {
              ...
              severity: "medium",
              enabled: true,
              config: { reporting_threshold: 10000 },
              created_at: new Date(),
              updated_at: new Date(),
            };

            const now = new Date();
            const prevTxs = sampleTxs.map((tx, idx) => ({
              investment_id: `inv_fc_${idx}`,
              investor_id: "fc_user",
              offering_id: "off1",
            ...
            const context: TransactionContext = {
              investment_id: "inv_fc_curr",
              investor_id: "fc_user",
              offering_id: "off1",
              amount: "9500",
              asset: "USD",
              timestamp: now,
              previous_transactions: prevTxs,
            };

            const results = await evaluator.evaluate(context, [rule]);
            const score = results[0].details.cluster_score as number;
  ...
  describe("Sanctions Screening Rule Evaluation", () => {
    const sanctionsRule: AMLRule = {
      ...
      description:
        "Screens investor against sanctions watchlist with Jaro-Winkler fuzzy matching",
    ...
    it("respects per-tenant threshold overrides in context.tenant_settings", async () => {
      ...
      const resultsStrict = await evaluator.evaluate(contextStrict, [
        sanctionsRule,
      ]);
...
describe("OFAC Counterparty Screening — Vessel & Aircraft", () => {
  ...
  const baseRule = (
    ...
    description:
      "Screens vessel/aircraft/organisation counterparties against OFAC SDN list",
  ...
  it("3: aircraft fuzzy name match → triggered=true, action=pending_review", async () => {
    ...
    const ctx = baseContext([{ name: "Fireberd One", type: "aircraft" }]);
    const [result] = await evaluator.evaluate(ctx, [baseRule()]);
  ...
  it("12: per-tenant threshold overrides rule config for counterparty fuzzy matching", async () => {
    ...
    const ctxPermissive: TransactionContext = {
      ...baseContext([{ name: "Arktika Ster", type: "vessel" }]),
      tenant_settings: { sanctions_threshold: 0.7 }, // permissive — should match
    };

    const [strictResult] = await evaluator.evaluate(ctxStrict, [baseRule()]);
    const [permissiveResult] = await evaluator.evaluate(ctxPermissive, [
      baseRule(),
    ]);
...
describe("Velocity Rule — Sliding Window Aggregation", () => {
  ...
  const velocityRule = (
    overrides: Partial<AMLRule["config"]> = {},
  ): AMLRule => ({
    id: "vel-rule-1",
  ...
  it("findByInvestor returns rows in descending window_end order", async () => {
    ...
    await velocityRepo.upsert({
      investor_id: "inv-1",
      window_start: new Date("2026-01-01T09:00:00Z"),
      window_end: t1,
      window_minutes: 60,
      tx_count: 1,
      total_amount: 100,
    ...
    await velocityRepo.upsert({
      investor_id: "inv-1",
      window_start: new Date("2026-01-01T11:00:00Z"),
      window_end: t2,
      window_minutes: 60,
      tx_count: 2,
      total_amount: 200,
    ...
    expect(rows[0].window_end.getTime()).toBeGreaterThan(
      rows[1].window_end.getTime(),
    );
  });
  ...
  it("findByInvestor filters by time range", async () => {
    await velocityRepo.upsert({
      investor_id: "inv-1",
      window_start: new Date("2026-01-01T09:00:00Z"),
      window_end: new Date("2026-01-01T10:00:00Z"),
      window_minutes: 60,
      tx_count: 1,
      total_amount: 100,
      investment_ids: ["a"],
    ...
    await velocityRepo.upsert({
      investor_id: "inv-1",
      window_start: new Date("2025-12-31T09:00:00Z"),
      window_end: new Date("2025-12-31T10:00:00Z"),
      window_minutes: 60,
      tx_count: 1,
      total_amount: 50,
      investment_ids: ["old"],
  ...
  it("labels metric with reason=amount when only amount exceeded", async () => {
    ...
    const counter = snap.custom.find(
      (p: any) =>
        p.name === "aml_velocity_triggered_total" &&
        p.labels?.reason === "amount",
    );
  ...
  describe("InMemoryVelocityRepository", () => {
    ...
    it("isolates records by investor_id", async () => {
      ...
      const rows = await repo.findByInvestor(
        "i1",
        new Date(0),
