...
| `src/middleware/session.ts` | Depends on `ISessionStore`; new secure cookie issuer (`buildSessionCookie`, `issueSessionCookie`, `clearSessionCookie`); login issues the cookie, logout clears it. Defaults to `SameSite=Strict`. |
...
- `src/middleware/session.test.ts` — auth middleware, session routes, login/logout, and stats.
- `src/db/repositories/sessionRepository.test.ts` — DB repository operations.
- `src/lib/__tests__/sessionStore.test.ts` — per-role TTL and idle extension metrics.
...
npx jest src/lib/postgresSessionStore.test.ts \
         ...
         src/lib/__tests__/sessionStore.test.ts
```
